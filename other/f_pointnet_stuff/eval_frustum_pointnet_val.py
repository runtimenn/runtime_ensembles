# evaluate F-pointnet on KITTI, record results in dict
...
network = FrustumPointNet("Frustum-PointNet_eval_val", project_dir=os.getcwd())
network.load_state_dict(torch.load("../../models/f_pointnet.pth"))
network = network.cuda()

NH = network.BboxNet_network.NH

val_dataset = EvalDatasetFrustumPointNet(kitti_data_path="../../data/kitti",
...
for step, (frustum_point_clouds, labels_InstanceSeg, labels_TNet, labels_BboxNet, labels_corner, labels_corner_flipped, img_ids, input_2Dbboxes, frustum_Rs, frustum_angles, centered_frustum_mean_xyz) in enumerate(val_loader):
    with torch.no_grad(): # (corresponds to setting volatile=True in all variables, this is done during inference to reduce memory consumption)
        ...
        outputs, patt = network(frustum_point_clouds)
        ...
        for i in range(outputs_InstanceSeg.size()[0]):
            ...
            if dont_care_mask_value == 1:
                ...
                bbox_dict['patt'] = patt[i].cpu().numpy()
...
with open('evaldict_val.pkl', 'wb') as handle:
    pickle.dump(eval_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
#%% load patts

#with open('evaldict_train.pkl', 'rb') as handle:
